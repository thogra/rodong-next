...
export function parseArticlePage(html, pageData) {
  ...
  const dateStr = $('.ArticleMenuDate').text();
  const datePart = _.head(_.split(dateStr, ','));
  const yearPart = dateStr.substring(_.indexOf(dateStr,'(')+1, _.indexOf(dateStr,')'));
  ...
  return {
    ...
    date: new Date(`${datePart} ${yearPart} 12:00`).toUTCString(),
    dateRaw: dateStr,
  };
}
